...
export const getL2PrecisionStatus = (task: Task): TorStatus => {
  ...
  const isFinished = !!task.realized_finish;
  const realizedDate = task.realized_finish ? startOfDay(new Date(task.realized_finish)) : null;
  ...
  // Not Finished (Realized Finish is empty)
...
export const checkPhaseSequencing = (tasks: Task[], currentTask: Task): boolean => {
  ...
  // A task is "Finished" if it has realized_finish
  const isFinished = (t: Task) => !!t.realized_finish;
...
export function calculateGlobalProjectStatus(tasks: Task[]) {
  ...
  const isCompleted = (task: Task) => {
    ...
    return !!task.realized_finish || status.includes('DONE') || status.includes('OVERDUE') || status.includes('EARLY');
  };
...
export const torAutoSync = async (projectId: string, tasks: Task[], projects: Project[], actor: string) => {
  ...
  const isTaskFinished = (t: Task) => !!t.realized_finish;
  const today = startOfDay(new Date());
  ...
  const newL1Updates: { id: string, realized_finish: string | null, status: TaskStatus }[] = [];
  ...
  for (const l1 of l1Tasks) {
    ...
    if (l2Tasks.length > 0) {
      ...
      if (allL2Finished && !l1.realized_finish) {
        ...
        const latestRealized = l2Tasks.reduce((latest, current) => {
          if (!latest) return current.realized_finish;
          if (!current.realized_finish) return latest;
          return new Date(current.realized_finish) > new Date(latest) ? current.realized_finish : latest;
        }, null as string | null);

        newL1Updates.push({ 
          id: l1.id, 
          realized_finish: latestRealized || new Date().toISOString(),
  ...
  for (const update of newL1Updates) {
    await supabase.from('tasks').update({ 
      realized_finish: update.realized_finish,
    ...
    await taskService.logAudit({
      ...
      newValue: { realized: update.realized_finish, note: `All L2 tasks finished` }
    });
  }
