...
export function TorMonitor({ user }: { user: any }) {
  ...
  const getProjectProgress = (pTasks: Task[]) => {
    ...
    const finished = pTasks.filter(t => !!t.realized_finish).length;
  ...
  const processedData: ProjectMonitorRow[] = projects.map(p => {
    ...
    const currentPhaseTask = l1Tasks.find(t => !t.realized_finish) || l1Tasks[l1Tasks.length - 1];
              ...
              <div className="space-y-6 relative z-10">
                ...
                <div className="flex flex-wrap gap-2">
                  {['FSD', 'DEV', 'SIT', 'UAT'].map(phase => {
                    ...
                    const isDone = !!l1Phase?.realized_finish;
